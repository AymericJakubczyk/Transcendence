...
class Pawn:
	...
	def setPossibleMoves(self, board, x, y):
		if (self.color == 'white'):
			if (y == 6 and board[y-1][x].piece == None and board[y-2][x].piece == None):
				board[y-2][x].possibleMove = True
				board[y-1][x].enPassant = True

			if (board[y-1][x].piece == None):
				board[y-1][x].possibleMove = True
			if (y - 1 >= 0 and x + 1 < 8 and board[y-1][x+1].piece != None and board[y-1][x+1].piece.color != self.color):
				board[y-1][x+1].possibleMove = True
			if (y - 1 >= 0 and x - 1 >= 0 and board[y-1][x-1].piece != None and board[y-1][x-1].piece.color != self.color):
				board[y-1][x-1].possibleMove = True
			...
			if (y == 1 and board[y+1][x].piece == None and board[y+2][x].piece == None):
				board[y+2][x].possibleMove = True
				board[y+1][x].enPassant = True

			if (board[y+1][x].piece == None):
				board[y+1][x].possibleMove = True
			if (y + 1 < 8 and x + 1 < 8 and board[y+1][x+1].piece != None and board[y+1][x+1].piece.color != self.color):
				board[y+1][x+1].possibleMove = True
			if (y + 1 < 8 and x - 1 >= 0 and board[y+1][x-1].piece != None and board[y+1][x-1].piece.color != self.color):
				board[y+1][x-1].possibleMove = True
...
class Rook:
	...
	def setPossibleMoves(self, board, x, y):
		for i in range(x + 1, 8):
			if (self.verif_loop_moove(board, self.color, i, y)):
				break
		for i in range(x - 1, -1, -1):
			if (self.verif_loop_moove(board, self.color, i, y)):
				break
		for j in range(y + 1, 8):
			if (self.verif_loop_moove(board, self.color, x, j)):
				break
